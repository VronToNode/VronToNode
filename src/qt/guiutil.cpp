...
void setupAddressWidget(QValidatedLineEdit* widget, QWidget* parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a VronToNode address (e.g. %1)").arg("XEBxEjGXmJxwpXxXAJqeXztdUR4MsZZJsE"));
#endif
...
{
    // return if URI is not valid or is no VronToNode: URI
...
{
    // Convert vrontonode:// to vrontonode:
    //
    //    Cannot handle this later, because vrontonode:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open vrontonode.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "VronToNode.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for VronToNode.lnk
...
{
    return GetAutostartDir() / "vrontonode.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a vrontonode.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=VronToNode\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the VronToNode app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add VronToNode app to startup item list
