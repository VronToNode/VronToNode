...
#if defined(NDEBUG)
#error "VronToNode cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in uvrontonode) are considered zero fee (for relaying and mining)
...
// vrontonode: total coin age spent in transaction, in the unit of coin-days.
...
CAmount GetBlockValue(int nHeight)
{
  // anti instamine
    ...
    if ( nHeight == 0 ) {
      nSubsidy = 400 * COIN;
	} else if (nHeight <= 1440*10 ) {
      nSubsidy = 400 * COIN;
	} else if (nHeight <= 1440*20 ) {
      nSubsidy = 200 * COIN;
	} else if (nHeight <= 1440*30 ) {
      nSubsidy = 200 * COIN;
	} else if (nHeight <= 1440*40 ) {
      nSubsidy = 40 * COIN;
	} else if (nHeight <= 1440*50 ) {
      nSubsidy = 10 * COIN;
	} else if (nHeight <= 1440*60 ) {
      nSubsidy = 10 * COIN;
	} else if (nHeight <= 1440*70 ) {
      nSubsidy = 40 * COIN;
	} else if (nHeight <= 1440*80 ) {
      nSubsidy = 35 * COIN;
	} else if (nHeight <= 1440*90 ) {
      nSubsidy = 30 * COIN;
	} else if (nHeight <= 1440*100 ) {
      nSubsidy = 20 * COIN;
	} else if (nHeight <= 1440*110 ) {
      nSubsidy = 22 * COIN;
	} else if (nHeight <= 1440*120 ) {
      nSubsidy = 25 * COIN;
	} else if (nHeight <= 1440*130 ) {
      nSubsidy = 28 * COIN;
	} else if (nHeight <= 1440*140 ) {
      nSubsidy = 27 * COIN;
	} else if (nHeight <= 1440*150 ) {
      nSubsidy = 25 * COIN;
	} else if (nHeight <= 1440*160 ) {
      nSubsidy = 23  * COIN;
	} else if (nHeight <= 1440*170 ) {
      nSubsidy = 21 * COIN;
	} else if (nHeight <= 1440*180 ) {
      nSubsidy = 20 * COIN;
	} else if (nHeight <= 1440*190 ) {
      nSubsidy = 19 * COIN;
    } else {
      nSubsidy = 18 * COIN;
    }
...
{
    RenameThread("vrontonode-scriptch");
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    // vrontonode: track money supply and mint amount info
...
CBlockIndex* AddToBlockIndex(const CBlock& block)
{
    ...
    if (miPrev != mapBlockIndex.end()) {
        ...
        // vrontonode: compute chain trust score
        pindexNew->bnChainTrust = (pindexNew->pprev ? pindexNew->pprev->bnChainTrust : 0) + pindexNew->GetBlockTrust();

        // vrontonode: compute stake entropy bit for stake modifier
        if (!pindexNew->SetStakeEntropyBit(pindexNew->GetStakeEntropyBit()))
            LogPrintf("AddToBlockIndex() : SetStakeEntropyBit() failed \n");

        // vrontonode: record proof-of-stake hash value
        if (pindexNew->IsProofOfStake()) {
        ...
        // vrontonode: compute stake modifier
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    // vrontonode: check proof-of-stake
